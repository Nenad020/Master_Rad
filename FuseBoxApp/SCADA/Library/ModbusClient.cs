namespace EasyModbus
{
	...
	public partial class ModbusClient
	{
		...
		public ModbusClient(string serialPort)
		{
			if (debug)
				StoreLogData.Instance.Store(
					"EasyModbus library initialized for Modbus-RTU, COM-Port: " + serialPort,
					DateTime.Now);
		...
		public void Connect()
		{
			...
			if (!udpFlag)
			{
				if (debug)
					StoreLogData.Instance.Store(
						"Open TCP-Socket, IP-Address: " + ipAddress + ", Port: " + port,
						DateTime.Now);
		...
		public static float ConvertRegistersToFloat(int[] registers)
		{
			...
			byte[] floatBytes =
				{
					lowRegisterBytes[0], lowRegisterBytes[1], highRegisterBytes[0], highRegisterBytes[1]
				};
		...
		public static Int64 ConvertRegistersToLong(int[] registers)
		{
			...
			byte[] longBytes =
				{
					lowRegisterBytes[0], lowRegisterBytes[1], lowHighRegisterBytes[0], lowHighRegisterBytes[1],
					highLowRegisterBytes[0], highLowRegisterBytes[1], highRegisterBytes[0], highRegisterBytes[1]
				};
		...
		public static int[] ConvertFloatToRegisters(float floatValue)
		{
			...
			int[] returnValue =
				{
					BitConverter.ToInt32(lowRegisterBytes, 0), BitConverter.ToInt32(highRegisterBytes, 0)
				};
			return returnValue;
		}
		...
		public static int[] ConvertIntToRegisters(Int32 intValue)
		{
			...
			byte[] highRegisterBytes = { doubleBytes[2], doubleBytes[3], 0, 0 };
			byte[] lowRegisterBytes = { doubleBytes[0], doubleBytes[1], 0, 0 };
			int[] returnValue =
				{
					BitConverter.ToInt32(lowRegisterBytes, 0), BitConverter.ToInt32(highRegisterBytes, 0)
				};
		...
		public static int[] ConvertLongToRegisters(Int64 longValue)
		{
			...
			byte[] highRegisterBytes = { longBytes[6], longBytes[7], 0, 0 };
			byte[] highLowRegisterBytes = { longBytes[4], longBytes[5], 0, 0 };
			byte[] lowHighRegisterBytes = { longBytes[2], longBytes[3], 0, 0 };
			byte[] lowRegisterBytes = { longBytes[0], longBytes[1], 0, 0 };
			int[] returnValue =
				{
					BitConverter.ToInt32(lowRegisterBytes, 0), BitConverter.ToInt32(lowHighRegisterBytes, 0),
					BitConverter.ToInt32(highLowRegisterBytes, 0), BitConverter.ToInt32(highRegisterBytes, 0)
				};
			return returnValue;
		}
		...
		public static int[] ConvertDoubleToRegisters(double doubleValue)
		{
			...
			byte[] highRegisterBytes = { doubleBytes[6], doubleBytes[7], 0, 0 };
			byte[] highLowRegisterBytes = { doubleBytes[4], doubleBytes[5], 0, 0 };
			byte[] lowHighRegisterBytes = { doubleBytes[2], doubleBytes[3], 0, 0 };
			byte[] lowRegisterBytes = { doubleBytes[0], doubleBytes[1], 0, 0 };
			int[] returnValue =
				{
					BitConverter.ToInt32(lowRegisterBytes, 0), BitConverter.ToInt32(lowHighRegisterBytes, 0),
		...
		private void DataReceivedHandler(object sender, SerialDataReceivedEventArgs e)
		{
			...
			if (debug)
				StoreLogData.Instance.Store(
					"Received Serial-Data: " + BitConverter.ToString(readBuffer),
					DateTime.Now);
			bytesToRead = 0;
		...
		public bool[] ReadDiscreteInputs(int startingAddress, int quantity)
		{
			...
			if (serialport != null)
				if (!serialport.IsOpen)
				{
					if (debug) StoreLogData.Instance.Store("SerialPortNotOpenedException Throwed", DateTime.Now);
					throw new Exceptions.SerialPortNotOpenedException("serial port not opened");
				}

			if (tcpClient == null & !udpFlag & serialport == null)
			{
				if (debug) StoreLogData.Instance.Store("ConnectionException Throwed", DateTime.Now);
				throw new Exceptions.ConnectionException("connection error");
			}

			if (startingAddress > 65535 | quantity > 2000)
			{
				if (debug) StoreLogData.Instance.Store("ArgumentException Throwed", DateTime.Now);
			...
			if (serialport != null)
			{
				...
				if (debug)
				{
					...
					if (debug)
						StoreLogData.Instance.Store(
							"Send Serial-Data: " + BitConverter.ToString(debugData),
							DateTime.Now);
				}
			...
			else if (tcpClient.Client.Connected | udpFlag)
			{
				...
				else
				{
					...
					if (debug)
					{
						...
						if (debug)
							StoreLogData.Instance.Store(
								"Send ModbusTCP-Data: " + BitConverter.ToString(debugData),
								DateTime.Now);
					}
					...
					if (ReceiveDataChanged != null)
					{
						...
						if (debug)
							StoreLogData.Instance.Store(
								"Receive ModbusTCP-Data: " + BitConverter.ToString(receiveData),
								DateTime.Now);
						ReceiveDataChanged(this);
					}
				}
			...
			if (data[7] == 0x82 & data[8] == 0x01)
			{
				...
				throw new Exceptions.FunctionCodeNotSupportedException(
					"Function code not supported by master");
			}

			if (data[7] == 0x82 & data[8] == 0x02)
			{
				if (debug) StoreLogData.Instance.Store("StartingAddressInvalidException Throwed", DateTime.Now);
				throw new Exceptions.StartingAddressInvalidException(
					"Starting address invalid or starting address + quantity invalid");
			}

			if (data[7] == 0x82 & data[8] == 0x03)
			{
				if (debug) StoreLogData.Instance.Store("QuantityInvalidException Throwed", DateTime.Now);
				throw new Exceptions.QuantityInvalidException("quantity invalid");
			}

			if (data[7] == 0x82 & data[8] == 0x04)
			{
				if (debug) StoreLogData.Instance.Store("ModbusException Throwed", DateTime.Now);
			...
			if (serialport != null)
			{
				...
				{
					if (debug) StoreLogData.Instance.Store("CRCCheckFailedException Throwed", DateTime.Now);
				...
				{
					if (debug) StoreLogData.Instance.Store("TimeoutException Throwed", DateTime.Now);
					if (NumberOfRetries <= countRetries)
					{
						countRetries = 0;
		...
		public bool[] ReadCoils(int startingAddress, int quantity)
		{
			if (debug)
				StoreLogData.Instance.Store(
					"FC1 (Read Coils from Master device), StartingAddress: " + startingAddress + ", Quantity: "
					+ quantity,
					DateTime.Now);
			transactionIdentifierInternal++;
			if (serialport != null)
			...
			else if (tcpClient.Client.Connected | udpFlag)
			{
				...
				else
				{
					...
					if (debug)
					{
						...
						if (debug)
							StoreLogData.Instance.Store(
								"Send MocbusTCP-Data: " + BitConverter.ToString(debugData),
								DateTime.Now);
					}
		...
		public void WriteSingleCoil(int startingAddress, bool value)
		{
			if (debug)
				StoreLogData.Instance.Store(
					"FC5 (Write single coil to Master device), StartingAddress: " + startingAddress + ", Value: "
					+ value,
					DateTime.Now);
			transactionIdentifierInternal++;
			if (serialport != null)
		...
		public void WriteSingleRegister(int startingAddress, int value)
		{
			if (debug)
				StoreLogData.Instance.Store(
					"FC6 (Write single register to Master device), StartingAddress: " + startingAddress + ", Value: "
					+ value,
					DateTime.Now);
			transactionIdentifierInternal++;
			if (serialport != null)
		...
		public void WriteMultipleCoils(int startingAddress, bool[] values)
		{
			...
			if (debug)
				StoreLogData.Instance.Store(
					"FC15 (Write multiple coils to Master device), StartingAddress: " + startingAddress + ", Values: "
					+ debugString,
					DateTime.Now);
			transactionIdentifierInternal++;
			...
			int[] values)
		{
			...
			if (debug)
				StoreLogData.Instance.Store(
					...
					+ startingAddressWrite + ", Values: " + debugString,
					DateTime.Now);
			transactionIdentifierInternal++;
		...
		{
			if (debug) StoreLogData.Instance.Store("Disconnect", DateTime.Now);
			if (serialport != null)
			{
		...
		~ModbusClient()
		{
			if (debug) StoreLogData.Instance.Store("Destructor called - automatically disconnect", DateTime.Now);
			if (serialport != null)
			{
				if (serialport.IsOpen)
					serialport.Close();
